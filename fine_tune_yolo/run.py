...
CAM1_VIDEO = os.path.join(_DATA, "cam1_half.mp4")
CAM2_VIDEO = os.path.join(_DATA, "cam2_half.mp4")
CAM1_MOT = os.path.join(_GT, "gt_cam1.txt")
CAM2_MOT = os.path.join(_GT, "gt_cam2.txt")

CAM3_VIDEO = os.path.join(_DATA, "cam3_half.mp4")
CAM3_MOT = os.path.join(_GT, "gt_cam3.txt")

# MOT class_id -> class name
CLASS_NAMES = {
    1: "car",
    2: "truck",
    3: "bus"
...
# Box filter (optional)
VISIBILITY_THRES = 0.0     # to drop blurry boxes, set 0.2 or 0.3
SKIP_IGNORED = True        # skip not_ignored == 0
...
def read_mot_file(mot_path):
    ...
    with open(mot_path, "r") as f:
        for line in f:
            ...
            if len(parts) < 9:
                # extra columns are fine, but skip rows with too few
                continue

            frame_id = int(float(parts[0]))
            # track_id = int(float(parts[1]))  # not needed for YOLO
            x = float(parts[2])
            y = float(parts[3])
            w = float(parts[4])
            h = float(parts[5])
...
def mot_box_to_yolo_line(class_id, x, y, w, h, img_w, img_h):
    ...
    # clamp (avoid out-of-bounds values)
    x_center = max(0.0, min(1.0, x_center))
    y_center = max(0.0, min(1.0, y_center))
    w_norm = max(0.0, min(1.0, w_norm))
    h_norm = max(0.0, min(1.0, h_norm))

    # drop boxes that are too small or invalid
...
def export_dataset(video_path, mot_path, out_root, prefix):
    """
    Build a train-only YOLO dataset:
    ...
    # OpenCV reads frames in order 1..N; we increment frame_idx ourselves
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        ...
        frame_idx += 1
        # file name from frame_id (6 digits)
        base_name = f"{prefix}_{frame_idx:06d}"
        img_path = os.path.join(images_dir, base_name + IMG_EXT)
        label_path = os.path.join(labels_dir, base_name + ".txt")

        # always write the image (so the dataset has every frame)
        # to keep only frames with a bbox, change this logic
        ...
        # create the label file (may be empty if there is no object)
        lines = []
        ...
        # YOLO accepts an empty label file (image with no object)
...
def write_data_yaml(out_root):
    """
    Write data.yaml into each dataset folder to make YOLO training easier.
    """
...
if __name__ == "__main__":
    ...
    print("\nAll done.")
    print("You can train YOLOv8, for example:")
